...
优化了业务层，让业务层不再负责输出，整体结构更好，更存粹，同时让agent生成了测试文件对功能进行了测试。

v0.4.0

沉寂已久，终于回归，用qt写了个简单的页面，实现了可交互性，可以做到选择文件路径，输入要查询的词汇，输出对应频率，很粗糙，但是是第一个小闭环，值得纪念。

v0.4.1

实现了拖拽上传文件，虽然说现在面对一次性上传一堆文件任然有问题，需要改进，但也算小的进步
下一步打算实现GUI层的模块划分，输入层，输出层，显示层，提示层

v0.4.2

实现了输出指定词语第 N 次出现位置的上下文，并支持自定义上下文的前文和后文范围。

当前版本已经能够正常完成主要功能，但仍有部分边界和异常情况暂未处理，例如：

查询的出现次数超过该词语的实际出现次数；
输入负数；
本应输入数字的位置输入了非数字内容。

这些情况将在下一个版本中进行处理，并进一步完善 TipPanel 的错误提示功能。